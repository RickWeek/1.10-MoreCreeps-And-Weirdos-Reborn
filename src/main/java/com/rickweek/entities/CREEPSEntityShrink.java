...
public class CREEPSEntityShrink extends EntityThrowable
{
    ...
    public void onUpdate()
    {
        ...
        if (movingobjectposition != null)
        {
                ...
                if (movingobjectposition.entityHit instanceof EntityLiving)
                {
                    ...
                    if (movingobjectposition.entityHit instanceof CREEPSEntitySneakySal)
                    {
                        if (((CREEPSEntitySneakySal)movingobjectposition.entityHit).modelsize > 0.6F)
                        {
                            ...
                            if (rand.nextInt(2) == 0)
                            {
                                worldObj.playSound((EntityPlayer) null, getPosition(), MCSoundEvents.ENTITY_SNEAKSAL_salnobodyshrinks, SoundCategory.PLAYERS, 0.5F, 1.0F);
                            }
                        }
                        else
                        {
                    ...
                    if (movingobjectposition.entityHit instanceof CREEPSEntityEvilChicken)
                    {
                        if (((CREEPSEntityEvilChicken)movingobjectposition.entityHit).modelsize > 0.5F)
                        {
                            ...
                            ((CREEPSEntityEvilChicken)movingobjectposition.entityHit).getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue(0.015D);
                        }
                        else
                        {
                    ...
                    /* TODO
                    if (movingobjectposition.entityHit instanceof CREEPSEntityMummy)
                    {
                        if (((CREEPSEntityMummy)movingobjectposition.entityHit).modelsize > 0.2F)
                        {
                            ((CREEPSEntityMummy)movingobjectposition.entityHit).modelsize -= 0.2F;
                        }
                        else
                        {
                            ((CREEPSEntityMummy)movingobjectposition.entityHit).setDead();
                            flag = true;
                        }
                    } 
                    ...
                    {
                        smoke();
                        worldObj.playSound((EntityPlayer) null, getPosition(), MCSoundEvents.SHRINKKILL, SoundCategory.PLAYERS, 1.0F, 1.0F / (rand.nextFloat() * 0.1F + 0.95F));
                        setDead();
                    }
                }
                else
            ...
            worldObj.playSound((EntityPlayer) null, getPosition(), MCSoundEvents.ITEM_RAYGUN, SoundCategory.PLAYERS, 0.2F, 1.0F / (rand.nextFloat() * 0.1F + 0.95F));
            setDead();
        }

        /* TODO
        if(worldObj.isRemote)
        	MCW.proxy.shrinkSmoke(worldObj, this);
		*/
