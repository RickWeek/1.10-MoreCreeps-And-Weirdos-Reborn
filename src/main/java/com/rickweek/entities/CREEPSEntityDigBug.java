...
public class CREEPSEntityDigBug extends EntityMob
{
    ...
    {
        super(world);
        texture = "mcw:textures/entity/digbug0.png";
        angerLevel = 0;
        attackRange = 16D;
        setSize(0.5F, 1.2F);
    ...
    public void onLivingUpdate()
    {
        ...
        {
            texture = (new StringBuilder()).append("mcw:textures/entity/digbug").append(String.valueOf(skinframe)).append(".png").toString();
            skinframe++;
            ...
            if (digtimer-- < 1)
            {
                ...
                */
                ...
                if (i3 != Blocks.SAND && i3 != Blocks.LOG)
                {
                    for (int j3 = 0; j3 < rand.nextInt(2) + 1; j3++)
                    {
                        EntityItem entityitem1 = new EntityItem(worldObj, (int)(holeX + xx), (int)((holeY - yy) + 1.0D), (int)(holeZ + zz), new ItemStack(i3, 1, 0));
        ...
        if (digstage == 2 && digtimer-- < 1)
        {
            ...
            for (int k1 = 0; k1 < list.size(); k1++)
            {
                ...
                {
                    entity.setDead();
                    double moveSpeed = this.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).getBaseValue();
                    moveSpeed = 0.4F;
        ...
        if (digstage == 3)
        {
            ...
            worldObj.playSound((EntityPlayer) null, getPosition(), MCSoundEvents.ENTITY_DIGBUG_EAT, SoundCategory.NEUTRAL, 1.0F, (rand.nextFloat() - rand.nextFloat()) * 0.2F + 1.0F);
            ...
            if (hunger-- < 1)
            {
                ...
                worldObj.playSound((EntityPlayer) null, getPosition(), MCSoundEvents.ENTITY_DIGBUG_FULL, SoundCategory.NEUTRAL, 1.0F, (rand.nextFloat() - rand.nextFloat()) * 0.2F + 1.0F);
            }
            else
            {
                digstage = 2;
    ...
    protected SoundEvent getLivingSound()
    {
        ...
        if (digstage == 1 || digstage == 4)
        {
            return MCSoundEvents.ENTITY_DIGBUG_DIG;
        }

        if (digstage == 2)
        {
            return MCSoundEvents.ENTITY_DIGBUG_CALL;
        }
        else
        {
            return null;
    ...
    {
        return MCSoundEvents.ENTITY_DIGBUG_HURT;
    }
    ...
    {
        return MCSoundEvents.ENTITY_DIGBUG_DEATH;
    }
