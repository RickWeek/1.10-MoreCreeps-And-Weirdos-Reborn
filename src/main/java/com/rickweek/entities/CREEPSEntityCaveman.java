...
public class CREEPSEntityCaveman extends EntityMob
{
    ...
    public void applyEntityAttributes()
    {
    	...
    	this.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue(0.230000005D);
    ...
    public void onUpdate()
    {
        ...
        if (wanderstate == 0 && frozen < 1 && !evil && !MCW.cavemanbuilding && rand.nextInt(100) == 0)
        {
            wanderstate = 1;
        }

        if (wanderstate == 1 && rand.nextInt(201) == 200 && !MCW.cavemanbuilding && !evil && checkArea())
        {
            wanderstate = 2;
            housechunk = 0;
    ...
    public boolean attackEntityFrom(DamageSource damagesource, float i)
    {
        ...
        if (entity != null && (entity instanceof EntityPlayer) && frozen > 0)
        {
            ...
            worldObj.playSound((EntityPlayer) null, getPosition(), MCSoundEvents.ENTITY_CAVEMAN_ICE, SoundCategory.NEUTRAL, 1.0F, (rand.nextFloat() - rand.nextFloat()) * 0.2F + 1.0F);
        ...
        if (frozen < 1)
        {
            ...
            if ((double)f < 2.8999999999999999D && entity.getEntityBoundingBox().maxY > getEntityBoundingBox().minY && entity.getEntityBoundingBox().minY < getEntityBoundingBox().maxY)
            {
                ...
                if (talkdelay-- < 0)
                {
                    ...
                    worldObj.playSound((EntityPlayer) null, getPosition(), MCSoundEvents.ENTITY_CAVEMAN_EVIL, SoundCategory.NEUTRAL, 1.0F, (rand.nextFloat() - rand.nextFloat()) * 0.2F + 1.0F);
                    talkdelay = 2;
                }
            }
    ...
    public void setCaveTexture()
    {
        if (evil)
        {
            if (cavegirl)
            {
                texture = "mcw:textures/entity/cavemanladyevil.png";
            }
            else
            {
                texture = "mcw:textures/entity/cavemanevil.png";
            }
        }
        else if (cavegirl)
        {
            texture = "mcw:textures/entity/cavemanlady.png";
        }
        else
        {
            texture = "mcw:textures/entity/caveman.png";
        }
    }
    ...
    protected SoundEvent getAmbientSound()
    {
        if (evil)
        {
            return MCSoundEvents.ENTITY_CAVEMAN_EVIL;
        }
        ...
        if (entityToAttack != null)
        {
            if (cavegirl)
            {
            	return MCSoundEvents.ENTITY_CAVEWOMAN_FREE;
            }
            else
            {
                return MCSoundEvents.ENTITY_CAVEMAN_FREE;
            }
        }
        ...
        if (cavegirl)
        {
            ...
            {
            	return MCSoundEvents.ENTITY_CAVEWOMAN_FROZEN;
            }
        }
        ...
        {
        	return MCSoundEvents.ENTITY_CAVEMAN_FROZEN;
        }
    }
    ...
    protected SoundEvent getHurtSound()
    {
        ...
        if (cavegirl)
        {
            return MCSoundEvents.ENTITY_CAVEWOMAN_HURT;
        }
        else
        {
        	return MCSoundEvents.ENTITY_CAVEMAN_HURT;
        }
    }
    ...
    protected SoundEvent getDeathSound()
    {
        if (cavegirl)
        {
            return MCSoundEvents.ENTITY_CAVEWOMAN_DEAD;
        }
        else
        {
        	return MCSoundEvents.ENTITY_CAVEMAN_DEATH;
        }
    }
    ...
    public void onDeath(DamageSource damagesource)
    {
        ...
        if(!worldObj.isRemote)
        {
            ...
            if (rand.nextInt(10) == 0)
            {
                dropItem(MCItems.Popsicle, rand.nextInt(3) + 1);
            }

            if (rand.nextInt(8) == 0)
            {
                dropItem(MCItems.CaveClub, 1);
            }
        }
